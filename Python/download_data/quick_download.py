...
def local_SubX_data(url, outdir, ftype, mod, inst, var, plev, yv, xv, ens, fen):
    ysave = str(int(yv))
    xsave = str(int(xv))
    inFname = url+'.'+inst+'/.'+mod+'/.'+ftype+'/.'+var+'/dods'
    remote_data = xr.open_dataarray(inFname)
    ...
    if len(remote_data.dims) == 6:
        da = remote_data.sel(P=plev, M=ens, Y=yv, X=xv)
    if len(remote_data.dims) == 5:
        da = remote_data.sel(M=ens, Y=yv, X=xv)
    ...
    outdir += ftype+'/'+mod+'/'+var+'/'+str(plev)+'/daily/ts/'
    if not os.path.isdir(outdir):
        os.makedirs(outdir)
    # Check if any files have been created so they are not downloaded again
    ...
    for ic in range(_icstart, da_S_len):
        ...
        if not np.all(np.isnan(da2)):
            ...
            if date.hour == 12:
                da2.S.values = da2.S.values - pd.Timedelta('12 hours')
...
def main():
	...
	if not (yv is None or xv is None):
		global_SubX_data(url, outdir, ftype, mod, inst, var, plev, ens, fen)
	else:
		local_SubX_data(url, outdir, ftype, mod, inst, var, plev, yv, xv, ens, fen)
